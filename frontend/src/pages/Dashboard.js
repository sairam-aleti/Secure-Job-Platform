  ...
  const setupEncryption = useCallback(async () => {
    ...
    try {
      ...
      if (existingEncryptedKey) {
        // Keys exist locally — just make sure the public key is in the backend.
        // This covers the case where the backend lost the key but localStorage kept it.
        ...
        // If decryption fails here, Login.js will handle it on next login — don't regenerate mid-session
      } else {
        // Truly no keys anywhere — generate fresh (old account or cleared localStorage)
          ...
          <>
            ...
            <motion.div className="card" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }}>
              ...
              <div className="upload-area">
                ...
                <input type="file" accept=".pdf,.docx" onChange={handleFileChange} disabled={resumes.length > 0} />
              </div>
              <button 
                    ...
                    <li key={app.id} className="resume-item">
                      ...
                      <div style={{ display: 'flex', gap: '8px' }}>
                        <button 
                          ...
                          onClick={() => navigate(`/chat/${app.recruiter_id}`)}
                        >
                          Chat with Recruiter
                          {unreadCounts.dm[app.recruiter_id] > 0 && (
...
//       setLoading(false);
//     }
//   }, [navigate, fetchResumes, fetchUserApplications, fetchRecommendations, fetchMyJobs, fetchRecruiterApplications, fetchPendingRequests, fetchMyConnections, fetchProfileViewers, setupEncryption]);
...
//                       <div className="resume-details">
//                         <span className="resume-name">{app.job_title}</span>
//                         <span className="resume-meta">Status: <strong style={{ color: 'var(--cy-brand)' }}>{app.status}</strong></span>
//                       </div>
//                       <div style={{ display: 'flex', gap: '8px' }}>
//                         <button 
//                           className="download-btn" 
